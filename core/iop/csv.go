...
func ReadCsvStream(path string) (ds *Datastream, err error) {
	...
	if err != nil {
		err = h.Error(err, "Invalid path: "+path)
		return
	}
...
func (c *CSV) Read() (data Dataset, err error) {
	ds, err := c.ReadStream()
	if err != nil {
		return data, h.Error(err, "Error reading csv stream")
	}
...
func (c *CSV) getReader() (*csv.Reader, error) {
	...
	if c.File == nil && c.Reader == nil {
		...
		if err != nil {
			return r, h.Error(err, "os.Open(c.Path)")
		}
		c.File = file
	...
	if err != nil {
		return r, h.Error(err, "Decompress(c.Reader)")
	}

	testBytes, reader2, err := h.Peek(readerDecompr, 0)
	if err != nil {
		h.LogError(err, "could not Peek")
		err = nil
	...
	if c.cleanup {
		// needs clean up
		h.Warn("end of line carriage returns detected. Cleaning up")
		...
		go func() {
			...
			if scanner.Err() != nil {
				h.LogError(h.Error(scanner.Err()))
			}
		}()
	} else {
		reader3 = reader2
	...
	if !c.NoTrace {
		h.Trace("delimiter chosen %#v", string(deli))
	}
...
func (c *CSV) ReadStream() (ds *Datastream, err error) {
	...
	r, err := c.getReader()
	if err != nil {
		return ds, h.Error(err, "Error getting CSV reader")
	}
	...
	if err == io.EOF {
		...
		return ds, h.Error(err, "could not read from Reader")
	}
...
func (c *CSV) WriteStream(ds *Datastream) (cnt uint64, err error) {
	...
	err = w.Write(fields)
	if err != nil {
		return cnt, h.Error(err, "error write row to csv file")
	}
...
func detectDelimiter(testBytes []byte) (bestDeli rune, numCols int) {
	bestDeli = ','
	deliList := []rune{',', '\t', '|', ';'}
	if os.Getenv("SLINGELT_DELIMITER") != "" {
		deliList = append([]rune(os.Getenv("SLINGELT_DELIMITER")), deliList...)
	}
	...
	for _, d := range deliList {
		...
		for {
			...
			if csvErr == io.EOF {
				...
				// h.Trace("failed delimited test for %#v: %s", string(d), err.Error())
				break
			}
			rowLens = append(rowLens, len(row))
		}
		if csvErr != nil && len(rowLens) < 3 {
			// h.Trace("csvErr for '%s': %s", string(d), csvErr.Error())
			continue
		}

		testCsvRowLens[d] = rowLens
		// h.Trace("rowLens for '%s': %#v", string(d), rowLens)
	}
