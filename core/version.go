...
func init() {
	...
	// or with build pin => branch.dev/2024-08-20.a1b2c3d (date + short SHA)
	...
	// check expiration date for dev build (30 day window); date is YYYY-MM-DD prefix
	dateStr := parts[1]
	if len(dateStr) >= 10 {
		dateStr = dateStr[:10]
	}

	if date := cast.ToTime(dateStr); !date.IsZero() && date.Add(30*24*time.Hour).Before(time.Now()) {
