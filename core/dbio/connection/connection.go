...
func (c *Connection) setURL() (err error) {
	...
	if strings.HasPrefix(c.URL(), "file://") {
		...
		if c.Type.IsDb() {
			...
			if !g.In(c.Type, dbio.TypeDbMotherDuck, dbio.TypeDbDuckDb, dbio.TypeDbDuckLake, dbio.TypeDbLanceDB, dbio.TypeDbSQLite, dbio.TypeDbD1, dbio.TypeDbBigQuery, dbio.TypeDbDynamoDB) {
